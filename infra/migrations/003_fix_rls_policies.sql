...
-- Version: 003
-- Date: 2025-11-08
-- Description: Simplifies RLS policies to remove circular dependencies
...
CREATE POLICY "Users can delete their own profile" ON users_profile
  FOR DELETE
  USING (auth.uid() = user_id);
...
CREATE POLICY "Users can view their own memberships" ON merchant_members
  FOR SELECT
  USING (user_id = auth.uid());

-- Merchant owners can insert members (direct owner check - NO JOIN to merchant_members)
CREATE POLICY "Owners can add members" ON merchant_members
  FOR INSERT
  WITH CHECK (
    EXISTS (
...
CREATE POLICY "Owners can update members" ON merchant_members
  FOR UPDATE
  USING (
    EXISTS (
      SELECT 1 FROM merchants
...
CREATE POLICY "Owners can delete members" ON merchant_members
  FOR DELETE
  USING (
    EXISTS (
      SELECT 1 FROM merchants
...
-- 1. Removing circular references between merchants and merchant_members
-- 2. Using only direct owner_user_id checks for merchant policies
-- 3. Simplifying all policy logic
